...
import { 
  ...
  Wand2, PieChart, Loader2, Sparkles
} from 'lucide-react';
import { generateWeeklyPlan, estimateMealCalories } from '../services/geminiService';
                  ...
                  <div className="flex-1 space-y-6">
                      {dailyMeals.length > 0 ? dailyMeals.map((meal, idx) => (
                          <div key={meal.id} className="relative group">
                              ...
                              <div className={`flex items-start gap-6 p-6 rounded-[2.5rem] border transition-all duration-300 ${meal.status === 'completed' ? 'bg-white dark:bg-slate-800/40 border-slate-100 dark:border-slate-800 shadow-sm' : 'bg-slate-50/50 dark:bg-slate-900/50 border-dashed border-slate-200 dark:border-slate-700'}`}>
                                  <div className={`p-4 rounded-2xl shrink-0 shadow-sm ${meal.status === 'completed' ? 'bg-primary-950 text-white' : 'bg-white dark:bg-slate-800 text-slate-400 border border-slate-100 dark:border-slate-700'}`}>
                                  ...
                                  <div className="flex-1 min-w-0">
                                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                                          <div>
                                              ...
                                              <h4 className="text-xl font-black font-serif text-slate-900 dark:text-white truncate pr-4">{meal.recipeTitle}</h4>
                                          </div>
                                          <div className="flex items-center gap-1">
                                              {meal.status === 'planned' ? (
                                                  <button onClick={() => onUpdateMealStatus && onUpdateMealStatus(meal.id, 'completed')} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all shadow-lg shadow-emerald-500/20">
                                              ...
                                              <button onClick={() => onDeleteMealLog && onDeleteMealLog(meal.id)} className="p-2 text-slate-200 hover:text-rose-500 transition-all opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
                                          </div>
                                      </div>
